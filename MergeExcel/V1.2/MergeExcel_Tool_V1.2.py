...
def main():
    ...
    def doProcess():
        ...
        wb1 = openpyxl.load_workbook(sModExcel)
        sheet_names = wb1.sheetnames
        ws1=wb1[sheet_names[0]]            #index为0是第一个sheet
        ...
        for i in range(max_cols):
            Dict_ModExcelCol[ws1.cell(row=1,column=i+1).value]=i
                    ...
                    for iCol1 in range(max_column):
                        if iCol1==0:
                            if old_List[iRow1] in last_List:                     #如果已有，则退出for循环，不增加重复数据
                                ...
                                #判断应该填写在哪一列，根据模板列来填写
                                iCol2=iCol1+1
                                ws1.cell(row=iRow2,column=iCol2).value=sheet.cell(iRow1,iCol1).value
                        else:
                            #判断应该填写在哪一列，根据模板列来填写
        ...
        sReportName="MergeResult_"+strTime+".xlsx"
